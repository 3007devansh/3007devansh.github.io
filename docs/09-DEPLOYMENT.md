...
**Build output:** `website/dist/` (13 MB, all static). Build command: `npm run build`.
...
> **Root directory is `website`** because the Astro project lives in the
> `website/` sub-folder, not the repo root.
...
1. On this machine: `cd website && npm run build`
2. Go to **https://app.netlify.com/drop**
3. Drag the **`website/dist`** folder onto the page.
...
4. **Update the site's own URL:** in `website/astro.config.mjs`, set
   `site: 'https://your-real-domain'`. This fixes canonical URLs, the sitemap
   and Open Graph links. Also update `website/public/robots.txt` (the Sitemap
